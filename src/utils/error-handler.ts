...
function buildInferredError(
  ...
  switch (code) {
    ...
    case 'ENCRYPTED_PDF':
      // 🔴 「暗号化 PDF をサポートしていません」とは書かない。読めないのは
      // **鍵が導けなかったから**であって、この server が暗号化を扱えないからではない。
      // 利用者パスワードが空の文書は復号して読んでいる。
      return makeError(
        code,
        'This PDF is password-protected, so its text could not be extracted.',
        {
          // 🔴 2026-08-31（S3）に文面を直した。それまでは「構造の観測は、
          // 暗号化されていない範囲について続けています」と書いていたが、
          // 続いていない —— 鍵が導けないと間接オブジェクトを 1 つも読めないので
          // （ADR-0008）、ページ数も分からない。読めていないことを
          // 「続けています」と書くのは、観測していないことに観測の顔をさせる。
          hint:
            '利用者パスワードが必要です。ISO 32000-2 §7.6.4.3.2 のとおり、' +
            'ファイル暗号鍵は利用者パスワードから導くため、パスワードが無いと' +
            'ファイル内の間接オブジェクトを 1 つも読めません（ページ数も含めて）。',
          detail: { cause: original.message },
        },
      );
    case 'FILE_TOO_LARGE':
