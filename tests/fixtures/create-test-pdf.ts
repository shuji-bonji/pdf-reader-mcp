...
/**
 * Linearized PDF fixture for Issue #1 regression
 * (`inspect_structure` / `inspect_fonts` must not throw on Linearized PDFs).
 *
 * Generated by piping `simple.pdf` through `qpdf --linearize`. If `qpdf` is not
 * available on the host, the fixture is skipped with a warning — the existing
 * `linearized.pdf` (if any) is left in place.
 */
async function createLinearizedPdf(): Promise<void> {
  const input = `${FIXTURES_DIR}/simple.pdf`;
  const output = `${FIXTURES_DIR}/linearized.pdf`;
  try {
    await execFileAsync('qpdf', ['--linearize', input, output]);
    console.log('Created: linearized.pdf (linearized via qpdf)');
  } catch (e) {
    const msg = e instanceof Error ? e.message : String(e);
    console.warn(`Skipped: linearized.pdf — qpdf not available or failed (${msg}).`);
  }
}
