...
| `read_text`      | テキスト抽出（Y座標ベースの読み順保持。`split_columns: 2 \| 3` で **タグなし** 多カラム PDF、`compact_whitespace` で 帳票 の U+3000 連続空白を畳み込み）。**生グリフ**を返すため、タグ付き PDF の本文は論理順 + `/ActualText` 解決の `extract_structured_text` を推奨 |
| `search_text`    | 全文検索（前後コンテキスト付き）。グリフレベルの検索のため `/ActualText` 置換で運ばれる語はヒットしない（タグ付き文書で 0 件のときは `note` で案内） |
...
「H1 の文字列は何か」に答えるツールです（座標順でフラットな `read_text` には
答えられません）。順序は構造木の深さ優先走査（ISO 32000-2 §14.8.2.5）。
`/ActualText` はグリフを置換し（§14.9.4）、`/Alt` は本文に混ぜず（§14.9.3）、
...
ページを跨いで続く表は **1 つの表**として返します — `pages` は配列で、
見出しも `## Table 3 — Pages 5–7` のようになります。要求した `pages` 範囲に
かかる表は丸ごと返り、セルの `/ActualText` 置換も解決します。
