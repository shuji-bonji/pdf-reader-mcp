...
**なぜ 2 段階に割ったか**: Issue 本文は UC-7 由来で「構造要素 → bbox」を求めているが、
**UC-10 が要るのは「オブジェクト ID → 座標」**で、必要な機構がまるで違う。
前者はコンテンツストリームを歩いてテキスト行列・グラフィックス状態を追う必要があるが、
後者は注釈の `/Rect` とページツリーで足りる。**先に UC-10 を閉じられる方を出した。**

**実装**: `src/services/object-locator.ts` + `src/tools/tier2/locate-objects.ts`（pdf-lib のみ）。

**設計上の決めどころ**:

| # | 決定 | 理由 |
| --- | --- | --- |
| 1 | 矩形は **writer `add_annotation` と同じ形**（`{x1,y1,x2,y2}`・左下原点・pt・正規化済み） | 受け渡しで座標系を解釈し直させない。ISO 32000-1 §7.9.5 が「対角 2 点を任意順で書いてよい/使用前に正規化」なので正規化は仕様側の要請でもある |
| 2 | **位置ごとに `basis` を持たせる**（`annotation-rect` / `page-box` / `page-content-stream` / `page-resource`） | 1 つの `rect` に潰すと、**ページ全体を指す矩形が「変更箇所の矩形」として読まれる**。強さの違う主張を同じ顔で返さない |
| 3 | 存在しない番号は **`found: false`**（「座標なし」ではない） | リビジョンで free されたオブジェクトが verify から渡ってくる。2 つを混ぜると「あるが位置不明」と誤読される |
| 4 | 暗号化文書では**座標と型は返し、`/T` は null** | 数値と名前は非暗号（ISO 32000-1 §7.6.2）。`ignoreEncryption` は復号しないので文字列は暗号文（既知の罠） |

**実測**: 手元の 60 PDF で error 0。実検体 `pdfnative-audit/out4/signed-then-annotated.pdf` で
verify の「署名後に追加された obj 27」→ **page 1 / rect (72,700,300,720)** = `add_annotation` にそのまま渡せる形。

**限界（出力に明記済み）**: コンテンツストリームは「ページ全体」までしか言えない。
XObject Form 内部のリソースは 1 段しか辿らない。

## #20 第 2 段階 — 構造要素 → bbox（UC-7・2026-07-27 実装）

**出力先**: 新ツールではなく **`extract_structured_text` の `include_bbox`**（既定 `false`）。
既存ツールが既に MCID → テキストを解決しているので追加コストが小さく、UC-7 が要る
「どの要素か＋本文＋どこか」が 1 回の呼び出しで揃う。**ツール数は 18 のまま**。

**実装**: `pdfjs-service.ts` の `runBox` / `buildIdToBoxMap` / `buildDocumentIdToBoxMap`
＋ `struct-tree-walker.ts` の `layoutBBoxOf`（`/A` と `/C`+`/ClassMap`）
＋ `struct-tree-service.ts` の `boxesOf`。

**「コンテンツストリームをグラフィックス状態付きで歩く」必要はなかった**。
pdfjs の `getTextContent({ includeMarkedContent: true })` が run ごとに
**Trm の平行移動＝ベースライン原点**と線形部（フォントサイズ・横倍率・回転）を
`transform` として返し、`styles[fontName]` が **ascent / descent** を返す。
第 1 段階の見立て（自前の走査が要る）は、テキストに関しては外れていた。

**設計上の決めどころ**:

| # | 決定 | 理由 |
| --- | --- | --- |
| 1 | `basis` は 2 値。`layout-attribute-bbox`（宣言）と `text-extent`（実測） | 種類の違う主張を同じ顔で返さない。前者は Table 379 の `/BBox`＝**生成者の申告**、後者は測った値。**テキストを持たない Figure について言えるのは前者だけ** |
| 2 | 矩形は**ページごとに 1 つ**（`boxes` 配列） | 1 つに潰すと、その要素が存在しないページに矩形を置くことになる（§14.8.2.5 NOTE 2） |
| 3 | 宣言はそのまま返した上で、**ページボックス**と**要素自身の本文**の両方に突き合わせ、矛盾を `boxNote` で報告。実測で黙って上書きしない | ファイルの自己矛盾は所見であって丸め誤差ではない。実測: 本文不一致は WTPDF 1.0 で 13 件中 10 件（全て `Table`）・PDF32000_2008 で 545 件中 98 件。**ページ外は WTPDF 1.0 で 1 件・PDF32000_2008 で 131 件** |
| 4 | 縦書き run を含む要素は**矩形を出さない**（`boxNote` で明示） | pdfjs は縦書きで advance を `width` でなく `height` に入れる。横書き分だけの矩形は「無い」より悪い（**間違っている**） |
| 5 | 空白のみの run を除外 | Table 379 が `BBox` を「**visible** content を完全に囲む矩形」と定義。実測: `spanning-table.pdf` の `TH` に隣の列まで届く 75pt の空白 run がある |
| 6 | 矩形は run 自身の軸（`(a,b)` / `(c,d)`）で組む | 軸並行に `x + width` すると回転テキストで**それらしく間違える**。45° / 20pt の実測で x2 が 317.8 → 正しくは 300.9 |

**独立した正解との突き合わせ（これが本命の検証）**: WTPDF 1.0 の `Link` **構造要素**
166 件の実測矩形を、生成者が同じリンクに置いた `Link` **注釈**の `/Rect` 173 件と比較。
**IoU 中央値 0.972・最小 0.460・完全に外れ 0 件**（`IoU>0.5` が 165/166）。
フィクスチャではなく、他人が独立に置いた座標と一致した。

**実機試用（ビルド＋再起動後）でしか出なかった発見**: WTPDF 1.0 の表紙 `Figure`（Creative Commons ロゴ）が
`/BBox [-32768 -32768 32767 32767]` を宣言している = **矩形であるべき場所に int16 のセンチネル**。
*Tagged PDF Best Practice Guide* の表紙にも同じ値があり（同一ツールチェーン）、
PDF32000_2008 では宣言 545 件中 **131 件**がページの外まで届く。
**無警告で返すと `add_annotation` に渡されて座標空間全体に描かれる** — この出力の売りが
「そのまま writer に渡せる」ことなので、無害な観測ではなく事故になる。
→ ページボックス（§7.7.3.3）との突き合わせを追加。フィクスチャにも同じセンチネルを実在値として入れた（BB-5b）。
**フィクスチャは実在の壊れ方を知らなかった**（[[green-tests-can-be-vacuous]] の型）。

**往復の実測（UC-7 ステップ 4 の閉じ）**: `element-bbox.pdf` の `P` の実測矩形
`(50, 297.52, 161.36, 308.62)` を **そのまま** writer `add_annotation` に渡し、
reader `inspect_annotations` で読み戻したところ `/Rect` は完全一致。
座標系の変換は 1 箇所も挟んでいない。

**不変条件（実測 78,198 要素で違反 0）**: 「本文がある = 位置もある」。
テキスト地図と矩形地図は同じ走査・同じ除外規則から作るので、
同一サーバ内で答えが割れてはいけない（#15 → #18 で一度やった失敗）。
検証: WTPDF 1.0 / Tagged-PDF Best Practice Guide / PDF32000_2008 / ISO 32000-2。

**コスト**: 通常文書で ±0%（pdfjs のページキャッシュが効く）、
49,760 要素の PDF32000_2008 で +40%、25,793 要素の ISO 32000-2 で +70%。既定 off。

**限界（出力の `bboxNotes` に明記済み）**:
実測は**行ボックス**（ascent/descent）であってグリフ輪郭ではない。
画像・ベクター描画は測っていない（宣言 `/BBox` が無ければ矩形なし）。
縦書きは測らない。
...
| v0.9.1 | **#18 = R-5 の本解決**（§14.9.4 の置換テキスト 2 経路をどちらも解決） |

いずれも npx でクリーン取得した公開版を実機で叩いて確認済み。**0.9.1 の実測（2026-07-20）**:
handshake `0.9.1`・17 ツール・`search_text("Difficult")` が **1 件**（0.9.0 は 0 件 + note）・
`read_text` も `Difficult` を返す = **同一サーバ内で答えが割れる状態は解消**。
