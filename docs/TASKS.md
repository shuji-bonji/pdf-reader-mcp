...
|------|------|
| 作成日 | 2026-07-16 |
| 最終更新 | 2026-07-27（**v0.10.0 = #20 第 1 段階 `locate_objects`**） |
| 現状 | **v0.10.0**・**18 ツール**（3 tier）。ローカル MCP で実検体試用 PASS。未処理は次メジャーでの `validate_*` 削除と、**#20 第 2 段階（構造要素 → bbox）** |
...
| **次の最優先** | **#20 第 1 段階（オブジェクト ID → 座標 = `locate_objects`）は v0.10.0 で出荷。** リリース後は npx 公開版で検証。**第 2 段階（構造要素 → bbox・UC-7 用）は未着手**（コンテンツストリームをグラフィックス状態付きで歩く必要があり、第 1 段階とは難度が違うので分離した） |
